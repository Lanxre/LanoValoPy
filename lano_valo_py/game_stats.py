...
class ValoGameStats:
    async def get_player_match_stats(
        ...
        stats = await calculate_stats(match.rounds, player, total_rounds)
...
async def calculate_stats(
    rounds: List[MatchRoundModel], player: MatchPlayerModel, total_rounds: int
) -> PlayerStatsModel:
    ...
    avg_damage_per_round = player.damage_made / total_rounds if total_rounds > 0 else 0
    kast = (
        ((stats.kills + stats.assists + (total_rounds - stats.deaths)) / total_rounds)
        * 100
        if total_rounds > 0
        else 0
    )
