import pytest
from lano_valo_py import LanoValoPy, GameApi
...
@pytest.mark.asyncio
async def test_get_player_cards(game_content: GameApi):
...
@pytest.mark.asyncio
async def test_get_player_card_by_uuid(game_content: GameApi):
...
@pytest.mark.asyncio
async def test_get_player_titles(game_content: GameApi):
...
@pytest.mark.asyncio
async def test_get_player_title_by_uuid(game_content: GameApi):
...
@pytest.mark.asyncio
async def test_get_maps(game_content: GameApi):
    options = GetPlayerTitleModel(language=Locales.ru_RU)
...
@pytest.mark.asyncio
async def test_get_map_by_uuid(game_content: GameApi):
    options = GetPlayerTitleModel(
